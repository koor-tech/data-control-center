...
func (m *Response) validate(all bool) error {
	...
	case *Response_ClusterStatusResponse:
		if v == nil {
		...
		if all {
			switch v := interface{}(m.GetClusterStatusResponse()).(type) {
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, ResponseValidationError{
						field:  "ClusterStatusResponse",
		...
		} else if v, ok := interface{}(m.GetClusterStatusResponse()).(interface{ Validate() error }); ok {
			if err := v.Validate(); err != nil {
